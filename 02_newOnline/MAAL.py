...
class AutonomousLooperOnline():
	...
	def __init__(self, 
				...
				BEATS_PER_LOOP=16,
				MIN_BEATS=16,
				MIN_LOOPS_REPETITION=4,
				MAX_LOOPS_REPETITION=32,
				sr=44100,
				fft_window=1024,
				fft_hopSize=512,
				RHYTHM_SUBDIVISIONS=16, 
				startup_similarityThreshold=0.003,  
				startup_repetition_numBars=4,  
		...
		self.MIN_LOOPS_REPETITION = MIN_LOOPS_REPETITION # minimun number of times a loop is repeated
		self.MAX_LOOPS_REPETITION = MAX_LOOPS_REPETITION # maximum number of times a loop is repeated
		self.LOOP_CHANGE_RULE = "better" # "better" OR "newer"
		if self.LOOP_CHANGE_RULE != "newer" and self.LOOP_CHANGE_RULE != "better":
			LOOP_CHANGE_RULE = "newer"
		self.STARTUP_MODE = "repetition" # "user-set" OR "repetition"
		if self.STARTUP_MODE != "user-set" and self.STARTUP_MODE != "repetition":
			self.STARTUP_MODE = "user-set"
		self.STARTUP_SIMILARITY_THR = startup_similarityThreshold
		self.N_BARS_STARTUP = startup_repetition_numBars # only relevant if STARTUP_MODE != "user-set"
		self.STARTUP_LOOP_BAR_N = 4 # first loop set by users
		self.TEMPO = BASE_BPM # bpm
		self.BEATS_PER_LOOP = BEATS_PER_LOOP # T_u
		self.MIN_BEATS_PER_LOOP = MIN_BEATS # T_l
		self.RHYTHM_SUBDIVISIONS = RHYTHM_SUBDIVISIONS # bar quantization
		...
		self.N_LOOPS = min(len(self.looping_rules),NUMBER_OF_LOOPS) # user might want less loops than those in the rules
...
if __name__ == '__main__': 
	...
	parser.add_argument('--NUMBER_OF_LOOPS', type=int, default=4,
						help='number of loop tracks')
	parser.add_argument('--BASE_BPM', type=int, default=120,
						help='base BPM for tempo')
	parser.add_argument('--BEATS_PER_LOOP', type=int, default=16,
						help='number of beats for each loop')
	parser.add_argument('--MIN_BEATS', type=int, default=16,
						help='minimum number of beats for loop selection')
	parser.add_argument('--MIN_LOOPS_REPETITION', type=int, default=4,
						help='minimum number of times a segment can loop before being subsituted')
	parser.add_argument('--MAX_LOOPS_REPETITION', type=int, default=32,
						help='maximum number of times a segment can loop before being dropped')
	parser.add_argument('--sr', type=int, default=44100,
						help='sample rate of the system')
	parser.add_argument('--fft_window', type=int, default=1024,
						help='number of samples for computation of fft')
	parser.add_argument('--fft_hopSize', type=int, default=512,
						help='hop size for fft computation')
	parser.add_argument('--RHYTHM_SUBDIVISIONS', type=int, default=16,
						help='subdivisions for computation of rhythms')
	parser.add_argument('--startup_similarityThreshold', type=float, default=0.003,
						help='threhsold for startup similarity')
	parser.add_argument('--startup_repetition_numBars', type=int, default=4,
						help='number of repetiotion for first bar computation')
	parser.add_argument('--ip', type=str, default='127.0.0.1',
						help='ip address of Pure Data (local)')
	parser.add_argument('--port_snd', type=int, default=6667,
						help='network port to send data to PD')
	parser.add_argument('--port_rcv', type=int, default=6666,
						help='network port to receive data from PD')
	parser.add_argument('--config_filepath', type=str, default='./config.json',
						help='path of the config file')
	parser.add_argument('--save_log_path', type=str, default='./',
						help='path where to save log of interactions')
	parser.add_argument('--verbose', type=int, default=1,
						help='written feedback from python')
	...
	# MIN_LOOPS_REPETITION = args.MIN_LOOPS_REPETITION
	# MAX_LOOPS_REPETITION = args.MAX_LOOPS_REPETITION
	# sr = args.sr
	# fft_window = args.fft_window
	# fft_hopSize = args.fft_hopSize
	# RHYTHM_SUBDIVISIONS = args.RHYTHM_SUBDIVISIONS
	# startup_similarityThreshold = args.startup_similarityThreshold
	# startup_repetition_numBars = args.startup_repetition_numBars
	# ip = args.ip
	# port_snd = args.port_snd
	# port_rcv = args.port_rcv
